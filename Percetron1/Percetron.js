var Percetron = ( function() {
	 ...
	 * Metodos privados
	 */
	Percetron.prototype.salidaObtenida = function( sinapsis ) {
		return sinapsis >= 0 ? 1 : 0;
	};

	Percetron.prototype.asignarPesos = function() {
		this.pesos = new Array( this.info[ 0 ].datos.length );
		
		var len = this.pesos.length, i,
			rango = this.rangoPesos.MAX - this.rangoPesos.MIN;

		for( i = 0; i < len; i++ ) {
			while( !this.pesos[ i ] )
				this.pesos[ i ] = parseFloat( ( Math.random() * rango + this.rangoPesos.MIN ).toFixed( 4 ) );
		}
	};

	Percetron.prototype.reajustarPesos = function( info ) {
		var len = this.pesos.length, i;
		
		for( i = 0; i < len; i++ ) {
			this.pesos[ i ] = this.pesos[ i ] + this.factorAprendizaje * info.error * info.datos[ i ];
		}
	};

	Percetron.prototype.calcularSinapsis = function( info ) {
		var len = this.pesos.length, i;
		info.sinapsis = 0;
	...
	Percetron.prototype.calcularError = function( info ) {
	 ...
	 * Getters y setters
	 */
	Percetron.prototype.getPesos = function() {
		return this.pesos;
	};
